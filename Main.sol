...
contract Context {
    ...
    function _msgData() internal view returns (bytes memory) {
        this; //静默状态可变警告而不生成字节码-请参阅https://github.com/ethereum/solidity/issues/2691
        return msg.data;
    }
}
...
contract InitializableERC20Detailed is IERC20 {
    ...
    /**
       *@dev设置“name”、“symbol”和“decimals”的值。他们三个
       *这些值是不可更改的：在执行过程中只能设置一次
       *建筑。
       *@注意避免变量阴影在参数名称后附加“Arg”。
       */
...
interface IMintRewardLogic{
    ...
    function sameUSDToRredeem (address addr,uint256 amt) external;
}
    ...
    InitializableReentrancyGuard
{
        ...
        initializer
    {
        ...
        interface_MintRewardLogic = IMintRewardLogic(_MintRewardLogic);
    ...
    modifier onlySavingsManager() {
        require(isCEO(), "Must be savings manager");
        _;
    }
    ...
    ****************************************/
        ...
        returns (uint256 quantityDeposited)
    {
        ...
        quantityDeposited = _quantity;

        //查看_bAsset 的小数点
        uint256 bAsset_decimals = CommonHelpers.getDecimals(_bAsset);
        uint256 amt = quantityDeposited*(1e18)/(10**bAsset_decimals);

        //增加sameusd存储占比
        interface_MintRewardLogic.sameUSDToMint(msg.sender,amt);
    ...
    ****************************************/
        ...
        returns (uint256 output)
    {
        ...
        //IERC20(_output).safeTransferFrom(outputDetails.integrator, _recipient, swapOutput);
...
*/
    ...
    ****************************************/
        ...
        returns (uint256 massetRedeemed)
    {
        ...
        uint256 fee = applyFee ? redemptionFee : 0;
    ...
    function _settleRedemption(
        ...
        for(uint256 i = 0; i < bAssetCount; i++){
            ...
            if(q > 0){
                ...
                IERC20(bAsset).safeTransfer( _recipient, q);
                //查看_bAsset 的小数点
                uint256 bAsset_decimals = CommonHelpers.getDecimals(bAsset);
                uint256 amt = q.mul(1e18).div(10**bAsset_decimals);
                interface_MintRewardLogic.sameUSDToRredeem(msg.sender,amt);
            }
        }
    }
    ...
    InitializableReentrancyGuard
{
     ...
     * @param CEOAddress            Address of CEOAddress
    ...
    ****************************************/
        ...
        returns (uint256 interestCollected, uint256[] memory gains)
    {
        ...
        for(uint8 i = 0; i < count; i++) {
            ...
            uint256 balance = IERC20(b.addr).balanceOf(integrations[i]);
